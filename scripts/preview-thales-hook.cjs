/* Aperçu PNG 1080x1920 — HOOK Thalès. PARIS / EST MAGIQUE, classe Tanaland,
   commentaire qui tacle l'OM, hook "l'erreur sur Thalès qui met ton prof en PLS". */
const sharp = require('sharp');
const fs = require('fs');

const W = 1080, H = 1920;
const BLEU = '#2342a8', ROUGE = '#d11f1f', PAPIER = '#fdeaf1';   // copie ROSE
...
// EN-TÊTE (jeu de mots PSG, noms empilés)
svg += txt(CL, 205, 'Nom : PARIS', {size:50, weight:700});
svg += txt(CL, 267, 'Prénom : EST MAGIQUE', {size:50, weight:700});
svg += txt(CL, 329, 'Classe : Tanaland', {size:50, weight:700});
...
svg += txt(bx+200, by+108, 'Tout est à revoir...', {size:40, fill:ROUGE});
svg += txt(bx+200, by+158, 'comme la saison de l\'OM.', {size:40, fill:ROUGE});

// HOOK
svg += txt(540, 720, "L'ERREUR SUR THALÈS", {size:74, fill:ROUGE, weight:700, anchor:'middle'});
svg += txt(540, 808, 'QUI MET TON PROF', {size:74, fill:ROUGE, weight:700, anchor:'middle'});
svg += txt(540, 896, 'EN PLS', {size:74, fill:ROUGE, weight:700, anchor:'middle'});
...
svg += `</svg>`;
fs.writeFileSync('/tmp/th.svg', svg);
sharp(Buffer.from(svg)).png().toFile('/home/user/anag-site/preview-thales-hook.png')
